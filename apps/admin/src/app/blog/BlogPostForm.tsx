  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-3xl">
      ...
      <div className="flex gap-3 pb-8">
        ...
        {form.slug && (
          <a
            href={`${process.env.NEXT_PUBLIC_SITE_URL ?? 'http://localhost:3000'}/api/draft/enable?slug=${form.slug}&secret=${process.env.NEXT_PUBLIC_DRAFT_SECRET ?? 'vps-draft-preview'}`}
            target="_blank"
            rel="noopener noreferrer"
            className="font-brand text-sm border border-brand-primary/20 text-brand-primary px-4 py-2.5 rounded-xl hover:bg-brand-cream transition-colors flex items-center gap-1.5"
          >
